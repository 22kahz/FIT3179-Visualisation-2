var vg_1 = "vegalitevis/map.vg.json";

vegaEmbed("#map", vg_1).then(function(result) {
}).catch(console.error);

var vg_2 = "vegalitevis/linechart.vg.json";

vegaEmbed("#linechart", vg_2).then(function(result) {
}).catch(console.error);

var vg_3 = "vegalitevis/sector.vg.json";

vegaEmbed("#sector", vg_3).then(function(result) {
}).catch(console.error);

var vg_4 = "vegalitevis/emissiontarget.vg.json";

vegaEmbed("#emissiontarget", vg_4).then(function(result) {
}).catch(console.error);

var vg_5 = "vegalitevis/temperature.vg.json";

vegaEmbed("#temperature", vg_5).then(function(result) {
}).catch(console.error);

var vg_6 = "vegalitevis/sector.vg.json";

vegaEmbed("#sector", vg_6).then(function(result) {
}).catch(console.error);

var vg_7 = "vegalitevis/emissionbygas.vg.json";

vegaEmbed("#emissionbygas", vg_7).then(function(result) {
}).catch(console.error);

var vg_8 = "vegalitevis/electricity.vg.json";

vegaEmbed("#electricity", vg_8).then(function(result) {
}).catch(console.error);